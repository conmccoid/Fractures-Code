...
def main(method='AltMin', linesearch=None, WriteSwitch=False, PlotSwitch=False):
    ...
    energies = np.zeros((loads.shape[0], 6))  # Initialize energies array
    ...
    if method!='AltMin':
        SNESKSP = KSPsetUp(fp, J, type="gmres", rtol=1.0e-5, max_it=1000)  # Set up the KSP solver
    ...
    for i_t, t in enumerate(loads):
        ...
        if WriteSwitch:
            with io.XDMFFile(fp.comm, f"output/EX_CTFM_{identifier}.xdmf","a") as xdmf:
                xdmf.write_function(fp.u, t)
                xdmf.write_function(fp.v, t)
        
    if WriteSwitch:
        with open(f"output/TBL_CTFM_{identifier}.csv",'a') as csv.file:
            writer=csv.writer(csv.file,delimiter=',')
