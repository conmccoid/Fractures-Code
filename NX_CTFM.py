...
def EX_save(method,linesearch):
    ...
    if method!='AltMin':
        ...
        with open(f"output/TBL_CTFM_{method}_{linesearch}_its.csv",'w') as csv.file:
            writer=csv.writer(csv.file,delimiter=',')
...
def EX_plot(linesearch_list):
    energies=np.loadtxt(f"output/TBL_CTFM_AltMin_energy.csv",delimiter=',',skiprows=1)
    ...
    for linesearch in linesearch_list:
        energies = np.loadtxt(f"output/TBL_CTFM_Newton_{linesearch}_energy.csv",delimiter=',',skiprows=1)
        output = np.loadtxt(f"output/TBL_CTFM_Newton_{linesearch}_its.csv",delimiter=',',skiprows=1)
    ...
    ax1[2].legend()
    plt.savefig(f"output/FIG_CTFM_its.png")
