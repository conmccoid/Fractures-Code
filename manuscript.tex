...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
	\State Find $\vec{u}_1 := \argmin E_\ell(\vec{u}, v_0)$, subject to boundary conditions
	\State Find $v_1 := \argmin E_\ell(\vec{u}_1, v)$, subject to boundary and irreversibility conditions
	\While{$\norm{v_1 - v_0} > \tau$}
...
\subsection{Crack-tip force method (CTFM)}
\label{subsec: ctfm}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{output/FIG_CTFM_energy.pdf}
	\caption{Energy for the CTFM example using the five methods tested.}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{output/FIG_CTFM_outerIts.pdf}
	\caption{Outer iterations (number of linear solves of the AltMin system) for each load step of the CTFM example.}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{output/FIG_CTFM_innerIts.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{output/FIG_CTFM_time.pdf}
	\caption{Wall clock time for each load step for the CTFM example.}
	\label{fig: ctfm time}
\end{figure}
...
\begin{table}
	...
	\end{tabular}
	\caption{Flop count for the CTFM example.}
	\label{tab: ctfm flops}
\end{table}
\begin{table}
	\centering
	\begin{tabular}{l | c c c}
	Method & Time (s) & \% AtlMin & \% MSPIN \\ \hline
	...
	\end{tabular}
	\caption{Timings for the CTFM example.}
	\label{tab: ctfm time}
\end{table}
