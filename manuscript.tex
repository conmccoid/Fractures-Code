...
\usepackage[backend=biber,style=authoryear,giveninits=true,
url=false, doi=true, eprint=true,date=year,isbn=false,natbib=true]{biblatex}
...
Instead, Francfort and Marigo \autocite{Francfort-Marigo-1998,bourdin2008variational} propose using a variational principle: the vector field of the displacement of the material minimizes an energy potential that combines elastic forces and the Griffith energy of all cracks \cite{griffith1921vi}.

This variational approach then requires calculating the Griffith energy, a challenge in practice.
...
[Recent advances]
In \cite{3k}, the authors applied the multiplicative Schwarz preconditioning inexact Newton method (MSPIN) to the phase-field model.
...
Multiplicative Schwarz preconditioning inexact Newton (MSPIN) was first described in \cite{cai}, with further explanation for field-split Schwarz methods in \cite{liu}.
MSPIN has been previously applied to phase-field fracture models in \cite{3k}.
...
\end{equation*}
In \cite{cai, liu, 3k}, the Jacobian for this inexact Newton's method is shown to be
\begin{equation*}
...
Cubic backtracking \cite{dennis1996numerical} performs this line search by repeatedly interpolating cubic polynomials and finding their minima.
...
\begin{algorithm}
	\caption{Cubic backtracking \cite[Algorithm A6.3.1, pg325-327]{dennis1996numerical} \\
...
The Docker image used for these experiments can be found on Docker Hub at \texttt{mccoidc/fractures-code:dolfinx} (nb: cite Docker and Apptainer).
Code for these experiments can be found on GitHub at \texttt{conmccoid/Fractures-Code}.

\subsection{Surfing boundary conditions}
\label{subsec: surf}

For this example, we begin with an initial crack of length 0.5 in an otherwise square domain of side length 6 with an unstructured grid of 4341 points.
...
This work was supported in parts by the Natural Sciences and Engineering Research Council of Canada (NSERC), RGPIN-2022-04536 and the Canada Research Chair program.
