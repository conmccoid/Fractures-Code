...
def main(method='AltMin',linesearch='bt',PlotSwitch=False,WriteSwitch=False):
    ...
    E_u, E_v, E_uu, E_vv, E_uv, E_vu, elastic_energy, dissipated_energy, total_energy = VariationalFormulation(u,v,dom)
    ...
    if WriteSwitch:
        with io.XDMFFile(dom.comm, f"output/EX_Test_{method}_{linesearch}.xdmf","w") as xdmf:
            xdmf.write_mesh(dom)
    ...
    for i_t, t in enumerate(loads):
        ...
        if WriteSwitch:
            with io.XDMFFile(dom.comm, f"output/EX_Test_{method}_{linesearch}.xdmf","a") as xdmf:
