...
def main(example='GD', method='AltMin', linesearch=None, maxit=100, WriteSwitch=False, PlotSwitch=False):
    if linesearch is None:
        identifier=f"{example}_{method}"
    else:
        identifier=f"{example}_{method}_{linesearch}"

    FPAltMin = importlib.import_module(f"FPAltMin_{example}")
    fp = FPAltMin.FPAltMin()
    ...
    if WriteSwitch:
        with io.XDMFFile(fp.comm, f"output/EX_{identifier}.xdmf","w") as xdmf:
            xdmf.write_mesh(fp.dom)
        with open(f"output/TBL_{identifier}.csv",'w') as csv.file:
    ...
    for i_t, t in enumerate(loads):
        ...
        if WriteSwitch:
            with io.XDMFFile(fp.comm, f"output/EX_{identifier}.xdmf","a") as xdmf:
                xdmf.write_function(fp.u, t)
                xdmf.write_function(fp.v, t)
    
    if WriteSwitch:
        with open(f"output/TBL_{identifier}.csv",'a') as csv.file:
            writer=csv.writer(csv.file,delimiter=',')
