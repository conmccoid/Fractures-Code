...
class NewtonSolverContext:
    ...
    def createVecs(self, mat):
        vec1=self.u.x.petsc_vec.duplicate()
        vec2=self.v.x.petsc_vec.duplicate()
        left=PETSc.Vec().createNest([vec1,vec2],None,MPI.COMM_WORLD)
        right=PETSc.Vec().createNest([vec1,vec2],None,MPI.COMM_WORLD)
        return left, right
